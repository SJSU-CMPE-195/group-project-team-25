...
def build_combined(train_rollouts: list[dict], eval_result: dict, out_path: Path):
    ...
    fig.suptitle("PPO+LSTM Bot Detection — Training & Evaluation Summary",
...
def main():
    ...
    parser.add_argument("--train-log", type=str, required=True, help="Path to training.log")
    parser.add_argument("--eval-log", type=str, required=True, help="Path to eval.log")
    parser.add_argument("--out", type=str, default="figures/combined.png",
    ...
    print(f"Training: {len(rollouts)} rollouts | Eval: acc={eval_result.get('accuracy', 0):.3f}")
